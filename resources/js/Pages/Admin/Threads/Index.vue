<template>
        ...
        <table class="min-w-full divide-y divide-gray-200 border rounded-lg">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-sm font-semibold text-gray-600">Content</th>
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 text-gray-600 text-sm">{{ thread.user?.name }}</td>
              <td class="px-6 py-4">
                <span class="px-3 py-1 text-xs  text-gray-600 rounded-full font-medium">
                  {{ thread.category?.name }}
                </span>
              </td>
              <td class="px-6 py-4 ">
              <button type="button" class="bg-red-500 rounded-2xl text-gray-300 py-1 px-2 text-sm" @click="openDeleteModal(thread)">
                Delete
              </button>
            </td>
  ...
  methods: {
    ...
    confirmDelete() {
      this.$inertia.delete(this.route('admin.threads.destroy', this.threadToDelete.id))
